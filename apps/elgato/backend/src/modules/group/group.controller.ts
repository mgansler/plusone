...
import { GroupStateRequestDto } from './dto/group-state-request.dto'
...
export class GroupController {
  ...
  @ApiBody({ description: 'Switches all devices in that group on or off.', type: GroupStateRequestDto })
  @ApiNoContentResponse({ description: 'All devices have been switched on or off.' })
  @Put('/:groupId/state')
  async controlGroupState(
    @Param('groupId', ParseIntPipe) groupId: number,
    @Body() groupStateInputDto: GroupStateRequestDto,
  ) {
